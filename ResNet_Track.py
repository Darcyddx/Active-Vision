...
class ResNet_BottleNeck(keras.layers.Layer):
    def __init__(self, filters, strides, decoder=False):
        super(ResNet_BottleNeck, self).__init__()

        self.bn_1 = keras.layers.BatchNormalization()
        self.active_1 = keras.layers.Activation("relu")
        self.conv_1 = keras.layers.Conv2D(
            filters, kernel_size=(1, 1), strides=1, padding="same",
        ...
        if not decoder:
            self.conv_3 = keras.layers.Conv2D(
                2 * filters, (1, 1), strides=1, padding="same",
                data_format="channels_last"
            )
        else:
            self.conv_3 = keras.layers.Conv2D(
                filters, (1, 1), strides=1, padding="same",
                data_format="channels_last"
            )
        ...
        if strides == 2:
            ...
            self.short_cut = keras.layers.Conv2D(
                filters, (1, 1), strides=1, padding="same",
                data_format="channels_last"
            )
...
class ResNet_Track(keras.models.Model):
    def __init__(self, input_shape, structure=[3, 3, 4, 3], num_filters=[16, 32, 64, 128]):
        ...
        self.inital = keras.Sequential(
            [
                keras.layers.Conv2D(64, (3, 3), padding="same", data_format="channels_last", input_shape=input_shape, kernel_initializer=kernel_initializer),
                keras.layers.BatchNormalization(),
        ...
        self.last = keras.Sequential(
            [
                ...
                keras.layers.BatchNormalization(),
                keras.layers.Activation("relu"),
                keras.layers.Conv2D(3, (3, 3), padding="same", data_format="channels_last", bias_initializer=keras.initializers.constant(-3.2)),
                keras.layers.Activation("sigmoid"),
            ]
        )

    def build_block(self, num_blocks, filters, strides, decoder=False):
        block = keras.Sequential()
        block.add(ResNet_BottleNeck(filters, strides=strides, decoder=decoder))
        for _ in range(num_blocks - 1):
            block.add(ResNet_BottleNeck(filters, strides=1, decoder=decoder))
        return block
...
if __name__ == "__main__":
    model = ResNet_Track(input_shape=(288, 512, 3))
    model.build(input_shape=(None, 288, 512, 3))
    # model.summary()

    dummy_input = tf.random.normal((2, 288, 512, 3)) 
